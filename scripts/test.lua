-- ==========================================
-- MORPH AVATAR PRO - VERSIÓN CON SELECTOR DE COPIA (v2.1.0)
-- Todas las correcciones de bugs y mejoras aplicadas
-- Incluye selector de qué copiar (ropa, accesorios, cara, piel, forma)
-- ==========================================
-- ==========================================
...
-- ✅ FUNCIÓN copyBodyObjects MEJORADA CON SOPORTE PARA FACE Y ACCESORIES
local function copyBodyObjects(target, options)
    ...
    if localDesc and options then
        -- Si la opción es FALSE, mantenemos lo local (NO copiamos del objetivo)
        ...
        if not options.accessories then
            targetDesc.Accessories = localDesc.Accessories
        end
        if not options.face then
            targetDesc.Face = localDesc.Face
        end
...
-- ==========================================
-- MODAL DE CONFIGURACIÓN DE COPIA (NUEVO)
-- ==========================================
local copyModal = createRoundedFrame(screenGui, UDim2.new(0, 320, 0, 380), UDim2.new(0.5, -160, 0.5, -190), COLORS.DARK_GRAY, 10)
copyModal.Visible = false
copyModal.ZIndex = 50
copyModal.BackgroundTransparency = 0.05

local modalTitle = Instance.new("TextLabel")
modalTitle.Size = UDim2.new(1, 0, 0, 45)
modalTitle.Text = "⚙️ Configurar Copia"
modalTitle.TextColor3 = COLORS.ACCENT
modalTitle.BackgroundTransparency = 1
modalTitle.Font = Enum.Font.GothamBold
modalTitle.TextSize = 18
modalTitle.TextXAlignment = Enum.TextXAlignment.Center
modalTitle.Parent = copyModal

local modalSubtitle = Instance.new("TextLabel")
modalSubtitle.Size = UDim2.new(1, -20, 0, 25)
modalSubtitle.Position = UDim2.new(0, 10, 0, 40)
modalSubtitle.Text = "Selecciona qué elementos copiar:"
modalSubtitle.TextColor3 = COLORS.LIGHT_GRAY
...
modalSubtitle.Parent = copyModal
...
local function createOptionCheck(parent, text, optionKey, yPos, tooltipText)
    ...
    local checkBtn = createButton(container, {
        Size = UDim2.new(0, 40, 0, 35),
        Position = UDim2.new(1, -40, 0, 2.5),
        Text = copyOptions[optionKey] and "✓" or "✕",
        TextSize = 18,
        Color = copyOptions[optionKey] and COLORS.LIGHT_GREEN or COLORS.MID_GRAY,
        Tooltip = tooltipText
    })

    connect(checkBtn.MouseButton1Click, function()
        copyOptions[optionKey] = not copyOptions[optionKey]
        checkBtn.Text = copyOptions[optionKey] and "✓" or "✕"
        checkBtn.BackgroundColor3 = copyOptions[optionKey] and COLORS.LIGHT_GREEN or COLORS.MID_GRAY
        flashButton(checkBtn)
    end)
end

createOptionCheck(copyModal, "👕 Ropa (Clásica/UGC)", "clothes", 75, "Copia camisas y pantalones")
createOptionCheck(copyModal, "🎩 Accesorios", "accessories", 120, "Copia sombreros, mochilas, capas, etc.")
createOptionCheck(copyModal, "😐 Cara (Face)", "face", 165, "Copia el rostro del avatar")
createOptionCheck(copyModal, "🎨 Color de Piel", "skin", 210, "Copia el color de todas las partes del cuerpo")
createOptionCheck(copyModal, "📐 Forma del Cuerpo", "shape", 255, "Copia escalas R15 (altura, ancho, etc.)")

local modalDivider = createRoundedFrame(copyModal, UDim2.new(1, -20, 0, 1), UDim2.new(0, 10, 0, 310), COLORS.MID_GRAY, 0)
modalDivider.BackgroundTransparency = 0.5

local confirmCopyBtn = createButton(copyModal, {
    Size = UDim2.new(0, 120, 0, 40),
    Position = UDim2.new(0.5, -65, 1, -50),
    Text = "✅ Copiar",
    TextSize = 15,
    Color = COLORS.LIGHT_GREEN,
    Tooltip = "Aplicar la copia con las opciones seleccionadas"
})

local cancelCopyBtn = createButton(copyModal, {
    Size = UDim2.new(0, 120, 0, 40),
    Position = UDim2.new(0.5, 55, 1, -50),
    Text = "❌ Cancelar",
    TextSize = 15,
    Color = COLORS.RED,
    Tooltip = "Cerrar sin copiar"
})

local pendingCopyTarget = nil
...
• ✅ 🎯 SELECTOR DE COPIA PERSONALIZADO (elige qué copiar)
...
• ✅ Modal de configuración de copia con 5 opciones
...
local function createPlayerCard(targetPlayer, isFavorite, showCopyOptions)
    ...
    local copyBtn = createButton(card, {
        ...
        Tooltip = "Copiar objetos (abre selector)"
    })
    ...
    -- ✅ MODIFICADO: Ahora abre el modal de configuración en vez de copiar directo
    connect(copyBtn.MouseButton1Click, function()
        flashButton(copyBtn)
        pendingCopyTarget = targetPlayer
        ...
        -- Actualizar visualmente los botones del modal
...
-- ✅ CONEXIONES DEL MODAL DE COPIA (NUEVO)
connect(confirmCopyBtn.MouseButton1Click, function()
    if pendingCopyTarget then
        copyModal.Visible = false
        copyBodyObjects(pendingCopyTarget, copyOptions)
        pendingCopyTarget = nil
    end
end)

connect(cancelCopyBtn.MouseButton1Click, function()
    copyModal.Visible = false
    pendingCopyTarget = nil
end)
...
loadFavorites()
sendNotification("Morph Avatar Pro", "By @sickly255 (SAGE) 🎨 v2.1.0", "")