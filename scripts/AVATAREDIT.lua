...
local function forceAccessoryOrder(character)
    -- Esperamos a que los accesorios terminen de cargarse
    task.wait(0.5)
    ...
    -- Ordenamos por nombre para consistencia (opcional, puedes cambiar el criterio)
    table.sort(accessories, function(a, b)
        return a.Name < b.Name
    end)
    
    -- Asignamos el Order property a cada accesorio
...
-- ❌ FUNCIÓN ELIMINADA: cleanCharacterAccessories
-- ApplyDescription ya maneja el reemplazo de accesorios automáticamente
...
local function morphToPlayer(target)
    ...
    -- ❌ ELIMINADO: cleanCharacterAccessories(character)
    -- ApplyDescription reemplaza accesorios automáticamente sin necesidad de borrarlos
    ...
    if applySuccess then
        ...
        -- ✅ Forzamos el orden de accesorios después del morph exitoso
...
local function copyBodyObjects(target, options)
    ...
    if applySuccess then
        ...
        -- ✅ Forzamos el orden después de copiar también
        task.spawn(forceAccessoryOrder, character)
    else
...
infoContentLabel.Size = UDim2.new(1, -30, 0, 0)  -- ✅ FIX: Solo restamos 30px (padding)
infoContentLabel.AutomaticSize = Enum.AutomaticSize.Y  -- ✅ Crece automáticamente
infoContentLabel.Position = UDim2.new(0, 0, 0, 0)  -- ✅ Posición desde 0
...
infoContentLabel.TextWrapped = true  -- ✅ Wrap activado
...
-- ✅ FIX: Función para actualizar CanvasSize dinámicamente
local function updateInfoCanvasSize()
    task.wait(0.1)  -- Esperar a que el texto se renderice
    local labelHeight = infoContentLabel.AbsoluteSize.Y
    local padding = 30  -- Top + Bottom padding
    infoContent.CanvasSize = UDim2.new(0, 0, 0, labelHeight + padding)
end

-- ✅ FIX: Actualizar cuando cambie el tamaño del label
infoContentLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateInfoCanvasSize)

-- ✅ FIX: Actualizar inicial con retry
task.spawn(function()
    for i = 1, 5 do  -- Reintentar hasta 5 veces
        task.wait(0.1)
...
loadFavorites()
sendNotification("🎭 Morph Avatar Pro", "By @sickly255 (SAGE) ✨ v2.1.5 | Fix Accesorios", "")