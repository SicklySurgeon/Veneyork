-- ==========================================
-- MORPH AVATAR PRO - VERSIÓN CON TEMAS (v2.1.5)
-- Sistema de 6 temas visuales integrado
-- FIX APLICADO: Orden de Accesorios + Eliminación de cleanCharacterAccessories
-- FIX VISUAL: Hex Input Layout Corregido
-- ==========================================
...
-- ==========================================
-- 🔧 NUEVA FUNCIÓN: FORZAR ORDEN DE ACCESORIOS
-- ==========================================
...
infoContentLabel.Text = [[📝 REGISTRO DE CAMBIOS (v2.1.5)
...
Versión con temas + fixes: v2.1.5
...
task.spawn(function()
    for i = 1, 5 do
        task.wait(0.1)
        if infoContentLabel.AbsoluteSize.Y > 0 then
            updateInfoCanvasSize()
            break
        end
    end
end)
...
-- ==========================================
-- PESTAÑA SEARCH - ✅ REDISEÑADA (Preview & Confirm)
-- ==========================================
...
-- ==========================================
-- 🎯 RESULTADO DE BÚSQUEDA (Posicionado Correctamente)
-- ==========================================

local searchResultContainer = createRoundedFrame(searchContent, 
    UDim2.new(1, 0, 0, 100), -- Altura ajustada
    UDim2.new(0, 0, 0, 120), -- ✅ FIX: Posicionado justo debajo del input (Y=120)
    COLORS.MID_GRAY, 
    8
)
searchResultContainer.Visible = false -- Oculto por defecto

local searchResultImage = Instance.new("ImageLabel")
searchResultImage.Size = UDim2.new(0, 70, 0, 70)
searchResultImage.Position = UDim2.new(0, 15, 0, 15)
...
searchResultName.Size = UDim2.new(1, -160, 0, 25)
searchResultName.Position = UDim2.new(0, 100, 0, 15)
...
searchResultId.Size = UDim2.new(1, -160, 0, 18)
searchResultId.Position = UDim2.new(0, 100, 0, 40)
searchResultId.Text = ""
...
searchResultStatus.Position = UDim2.new(1, -95, 0, 42)
...
-- ✅ NUEVO BOTÓN DE CONFIRMACIÓN DENTRO DE LA TARJETA
...
-- Botón de Búsqueda (Reemplaza al antiguo Morph Now)
local searchBtn = createButton(searchContent, {
    Size = UDim2.new(1, 0, 0, 45),
    Position = UDim2.new(0, 0, 0, 230), -- ✅ Movido debajo de la tarjeta de resultado
    Text = "🔍 Search Player",
    TextSize = 16,
    Color = COLORS.BLUE
})
...
local idMorphBtn = createButton(searchContent, {
    ...
    Position = UDim2.new(0, 0, 0, 285), -- ✅ Movido más abajo
    Text = "🆔 Morph by ID",
    TextSize = 14,
    Color = COLORS.MID_GRAY
})
...
searchInfoLabel.Size = UDim2.new(1, 0, 0, 60)
searchInfoLabel.Position = UDim2.new(0, 0, 1, -50)
...
local function showSearchResult(targetPlayer)
    ...
    -- ✅ FIX: Detectar si es una Tabla (Offline) o una Instancia (Online)
    if typeof(targetPlayer) == "Instance" then
        -- Es un objeto Player real (Online)
        userId = targetPlayer.UserId
        playerName = targetPlayer.Name
        displayName = targetPlayer.DisplayName or playerName
        isOnline = true
    elseif typeof(targetPlayer) == "table" then
        -- Es una tabla de datos (Offline)
        userId = targetPlayer.UserId
        playerName = targetPlayer.Name or "Unknown"
        displayName = targetPlayer.DisplayName or playerName
        isOnline = false
    else
        -- Tipo desconocido, abortar para evitar crash
        sendNotification("❌ Error", "Tipo de jugador no reconocido", "")
        return
    end
    
    -- Actualizar UI
    searchResultName.Text = playerName
    searchResultDisplay.Text = "@" .. displayName
...
-- ✅ FIX VISUAL: Contenedor hex mejorado
...
-- ✅ FIX VISUAL: Input más pequeño y mejor posicionado
...
-- ✅ FIX VISUAL: Botón Aplicar mejor posicionado
...
loadFavorites()
sendNotification("🎭 Morph Avatar Pro", "By @sickly255 (SAGE) ✨ v2.1.5 | Fix Accesorios + Hex Visual", "")