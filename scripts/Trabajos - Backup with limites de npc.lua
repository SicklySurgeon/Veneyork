-- ==========================================
-- AUTO-JOB DELIVERY SYSTEM + PANEL HORIZONTAL (v5 DISTANCE CHECK)
-- ==========================================
...
local maxNpcDistance = 1000         -- Slider: 50 - 1000000 (Default: 5000)
...
-- ==========================================
-- 1. GUI HORIZONTAL (PANEL REORGANIZADO)
-- ==========================================
...
main.Name = "MainPanel"
main.Size = UDim2.new(0, 680, 0, 200) -- Altura aumentada para nuevo slider
...
content.Size = UDim2.new(1, -20, 0, 155)
...
local toggleCooldown = createToggle(content, "Prompt Cooldown", true, UDim2.new(0, 500, 0, 35))

local sliderSpeed    = createSlider(content, "Velocidad (Studs)", 10, 50, 25, UDim2.new(0, 0, 0, 35))
local sliderAnti     = createSlider(content, "Intentos Anti-Bucle", 1, 5, 3, UDim2.new(0, 250, 0, 35))
local sliderNpcDist  = createSlider(content, "Distancia Máx NPC", 50, 10000, 5000, UDim2.new(0, 0, 0, 80))
...
-- ==========================================
-- 4. SISTEMA ANTI-BUCLE / REGENERACIÓN
-- ==========================================
local function triggerAntiLoopReset()
    systemPaused = true
    infoLabel.Text = "🔄 Pausando trabajo. Regenerando personaje..."
    ...
    lockedGiverPrompt = nil
    systemPaused = false
    infoLabel.Text = "✅ Personaje regenerado. Reanudando trabajo..."
...
-- ==========================================
-- 5. CICLO PRINCIPAL (CON CHECK DE DISTANCIA NPC)
-- ==========================================
function runAutoJob()
    while autoJobEnabled do
        ...
        if not hasChicken() then
            ...
            if npcPos and npcModel then
                -- 📏 NUEVO: VERIFICACIÓN DE DISTANCIA ANTES DE TELEPORTAR
                local distToNpc = (npcPos - root.Position).Magnitude
                if distToNpc > maxNpcDistance then
                    infoLabel.Text = string.format("🚫 NPC muy lejos (%.0f studs). Regenerando...", distToNpc)
                    infoLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
                    triggerAntiLoopReset()
                    continue
                end

                infoLabel.Text = string.format("📍 NPC detectado: %.1f, %.1f, %.1f (%.0f studs)", npcPos.X, npcPos.Y, npcPos.Z, distToNpc)
                infoLabel.TextColor3 = Color3.fromRGB(0, 255, 150)
...
sliderNpcDist.SetCallback(function(v) maxNpcDistance = v end)
