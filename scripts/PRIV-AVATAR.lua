-- ==========================================
-- MORPH AVATAR PRO - VERSION CON TEMAS (v2.1.3)
...
-- ? FUNCION DE HISTORIAL SIN DUPLICADOS
...
-- ??? FUNCION DE VALIDACION CON PROTECCION
...
-- ?? FUNCION DE MORPH SEGURO (Previene clips/caidas)
local function morphToPlayer(target)
    ...
    -- ??? Guardar posicion actual para evitar clips/caidas
    local savedCFrame = rootPart.CFrame
    local savedAnchor = rootPart.Anchored
    rootPart.Anchored = true -- Anclar temporalmente para evitar fisica durante el morph
...
??? Boton ?? en la barra de titulo para cambiar temas rapidamente
??? Vista previa de avatar mejorada (muestra thumbnail al hacer clic)
...
?? MORPH SEGURO: Previene caidas, clips y regeneracion forzada

??? INTERFAZ DE USUARIO
?? 6 temas visuales intercambiables sin reiniciar
?? Diseno moderno de alto contraste (Corregido tema claro)
...
?? Advertencias visibles sobre bugs conocidos (Espanol)
? Boton 'Reset' eliminado
...
? Arreglado: Texto visible en barra de busqueda (tema Light)
? Arreglado: Texto de advertencia en espanol
...
-- ?? ADVERTENCIA DE BUG CONOCIDO (Reducida altura y en Espanol)
...
searchWarningLabel.Text = "?? Bug conocido: Copiar avatares dentro de la plaza puede enviar al jugador bajo el piso debido a la nueva colision de vehiculos"
...
searchLabel.Position = UDim2.new(0, 0, 0, 45) -- ? AJUSTADO POR QUE WARNING ES MAS PEQUENO
...
-- ? ELIMINADO: Boton Reset to Original
...
playersWarningLabel.Text = "?? Bug conocido: Copiar avatares dentro de la plaza puede enviar al jugador bajo el piso debido a la nueva colision de vehiculos"
...
playersScrollFrame.Size = UDim2.new(1, 0, 1, -85) -- ? AJUSTADO POR WARNING MAS PEQUENO
...
-- ? ELIMINADO: Conexion del boton Reset
