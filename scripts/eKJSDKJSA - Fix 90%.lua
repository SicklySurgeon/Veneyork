-- ==========================================
-- AUTO-JOB DELIVERY SYSTEM + PANEL HORIZONTAL (v5 TELEPORT FIX)
-- ==========================================
...
local function runAntiAfk()
    while antiAfkEnabled do
        task.wait(math.random(580, 620))
        pcall(function()
            if keypress and keyrelease then
                keypress("w") task.wait(0.05) keyrelease("w")
...
-- ✅ TELEPORT ROBUSTO (CORREGIDO: Anclaje seguro, sin lucha física, sync de red)
local function smoothTeleport(targetPos)
    ...
    local traveled = 0
    local maxSteps = math.ceil(totalDist / step) + 15 -- 🔒 Límite de seguridad anti-bucle
    local steps = 0

    local wasAnchored = root.Anchored
    root.Anchored = true -- Previene rollback del servidor durante el movimiento

    local success = true
    while traveled < totalDist and steps < maxSteps do
        ...
        traveled += move
        steps += 1
        task.wait(0.01) -- Yield estable y ligero
    end
    ...
    -- 🔓 Restaurar estado original SIEMPRE (incluso si falla)
    root.Anchored = wasAnchored

    -- ⏳ Esperar replicación del servidor y estabilización física
    task.wait(0.08)

    -- ✅ Verificación con tolerancia realista (replicación + física)
    local finalRoot = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if not finalRoot or (finalRoot.Position - targetPos).Magnitude > 15 then
        return false
    end
    return success
end
